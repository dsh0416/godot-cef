...
CefTexture 与隐藏的 LineEdit 共用一个逻辑浏览器焦点。再次点击可编辑元素时，
Godot 焦点转移到代理，不会使 CEF 失去焦点。点击其他 Godot 控件后，浏览器会
交出焦点；延迟到达的输入法事件不会将焦点抢回。浏览器或窗口失去焦点、控件
被隐藏或暂停时，尚未完成的输入法组合会被取消。

指针事件遵循 Godot 的 GUI 命中测试、`mouse_filter`、坐标变换和捕获规则。
在浏览器内按下后，即使移到控件外，浏览器仍能收到移动和释放事件。键盘事件
仅转发给拥有焦点的浏览器。传入的 Godot 事件不会被修改，因此多个浏览器控件
可以与原生控件共存。手动向 `CefTexture2D` 转发事件的使用方仍需自行管理焦点
和指针路由。
