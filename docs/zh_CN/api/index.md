...
所有 `CefTexture` 和 `CefTexture2D` 实例共用同一个 CEF 任务调度服务，
到期时在每次 Godot 主循环迭代中至多执行一次。暂停场景、隐藏浏览器或
释放最后一个浏览器都不会断开该服务。浏览器渲染和节点信号仍遵循各自的
处理设置。

该服务依赖 Godot 的 `SceneTree` 主循环，并在其主线程运行。
全局帧率限制、窗口最小化后的降频和阻塞主线程的操作都会延迟 CEF 任务；
浏览器帧率设置不能绕过这些限制。
