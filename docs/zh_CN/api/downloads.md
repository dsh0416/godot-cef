...
Godot CEF 提供对网页发起的文件下载的完全控制。当用户点击下载链接或页面触发下载时，CEF 会向 Godot 发出信号而不是自动下载文件。这使您可以完全控制下载过程。
...
::: warning
下载**不会**自动开始。您必须处理 `download_requested` 信号并决定如何处理每个下载。这是一个防止不需要的文件下载的安全功能。
:::
